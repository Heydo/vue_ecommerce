...
export default {
  ...
  methods: {
    ...
    async userStateChanged (userinfo) {
      ...
      // 如果操作失败就提示用户，并且将用户状态取反以保证页面数据没有变化
      if (res.meta.status !== 200) {
    ...
    async saveRoleInfo () {
      ...
      if (res.meta.status !== 200) {
        return this.$message.error('为用户更新角色失败')
      }
      this.$message.success('为用户更新角色成功')
      this.getUserList()
